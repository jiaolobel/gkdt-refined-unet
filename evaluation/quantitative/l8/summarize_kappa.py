...
class GlobalConfig:
    def __init__(self):
        ...
        self.file_path = os.path.join(task)
        self.save_name = os.path.join(task, "kappas.csv")
...
for f in config.files:
    acc = open(os.path.join(config.file_path, f, 'kappa.txt')).readlines()[0]
    scores.append(acc)

scores = np.asarray(scores)

dframe = pd.DataFrame(scores)
