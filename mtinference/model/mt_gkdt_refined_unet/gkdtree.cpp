...
void GKDTFilter::seqcompute(const float *val, int vd, const float *pos,
                            float *out) {
    ...
    vector<float> leafValues(tree_->nLeaves() * vd, 0.0f);

    // Splatting: For each position vector ...
    for (int i = 0; i < n_; i++) {
        // find up to 4 leaves nearby ...
        int results =
            tree_->lookup(pos + i * pd_, &indices[0], &weights[0], inleaf);
        // and scatter to them.
    ...
    memset(out, 0, sizeof(float) * n_ * vd);

    // Slicing: For each position vector ...
    for (int i = 0; i < n_; i++) {
        // find up to 64 leaves nearby ...
        int results =
            tree_->lookup(pos + i * pd_, &indices[0], &weights[0], outleaf);
...
GKDTFilter::GKDTFilter(int pd, int n, int nthreads) {
    pd_ = pd;
    n_ = n;
...
void GKDTFilter::seqinit(const float *pos) {
    ...
    tree_ = new GKDTree(pd_, &points[0], points.size(), sqrtf(2.0f));
}

void GKDTFilter::mtcompute(const float *val, int vd, const float *pos,
                           float *out) {
    int inleaf = 4, outleaf = 64;
    ...
    // The values stored at the leaves
    float *leafValues = new float[tree_->nLeaves() * vd];
    float *mtleafValues = new float[nthreads_ * tree_->nLeaves() * vd];
    memset(leafValues, 0, sizeof(float) * tree_->nLeaves() * vd);
    memset(mtleafValues, 0, sizeof(float) * nthreads_ * tree_->nLeaves() * vd);

    auto tsplat = [&](int threadi) {
        int *tindices = indices + threadi * inleaf;
        float *tweights = weights + threadi * inleaf;
        float *tleafValues = mtleafValues + threadi * tree_->nLeaves() * vd;
        const float *tpos = pos + threadi * tn_ * pd_;
        const float *tval = val + threadi * tn_ * vd;
        ...
        for (int i = 0; i < tn_; i++) {
            ...
            for (int j = 0; j < results; j++) {
                for (int k = 0; k < vd; k++) {
                    tleafValues[tindices[j] * vd + k] +=
                        tval[i * vd + k] * tweights[j];
                }
            }
        }
    };
    ...
    // Reduce leaf values
    for (int i = 0; i < nthreads_; i++) {
        float *tleafValues = mtleafValues + i * tree_->nLeaves() * vd;
        for (int j = 0; j < tree_->nLeaves(); j++) {
            for (int v = 0; v < vd; v++) {
                leafValues[j * vd + v] += tleafValues[j * vd + v];
            }
        }
    }
    delete[] mtleafValues;
    ...
    auto tslice = [&](int threadi) {
        ...
        for (int i = 0; i < tn_; i++) {
            ...
            for (int j = 0; j < results; j++) {
                for (int k = 0; k < vd; k++) {
                    tout[i * vd + k] +=
                        leafValues[tindices[j] * vd + k] * tweights[j];
                }
            }
        }
    };
