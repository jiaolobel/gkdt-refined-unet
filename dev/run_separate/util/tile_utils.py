...
def reconstruct_full(tiles: np.ndarray, crop_height: int, crop_width: int) -> np.ndarray:
    ...
    def reconstruct(tiles, num_height, num_width, crop_height, crop_width):
        ...
        if tiles.ndim == 3:
            full = np.ndarray(shape=(num_height * crop_height, num_width * crop_width), dtype=tiles.dtype)
        else:
            n_channels = tiles.shape[3]
            full = np.ndarray((num_height * crop_height, num_width * crop_width, n_channels), dtype=tiles.dtype)
