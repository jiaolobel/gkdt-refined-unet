...
DenseCRF::DenseCRF(int H, int W, int n_classes, int d_bifeats, int d_spfeats,
    ...
    bilateral_filter_ = new GKDTFilter(d_bifeats_, N_, n_thread_);
    spatial_filter_ = new GKDTFilter(d_spfeats_, N_, n_thread_);
...
void DenseCRF::seqinference(const float *unary, const float *reference,
    ...
    bilateral_filter_->seqinit(bilateral_feats);
    spatial_filter_->seqinit(spatial_feats);
    ...
    bilateral_filter_->seqcompute(allOnes, 1, bilateral_feats,
                                  bilateral_norm_vals);
    spatial_filter_->seqcompute(allOnes, 1, spatial_feats, spatial_norm_vals);
    delete[] allOnes;
    ...
    for (int i = 0; i < n_iterations_; i++) {
        ...
        bilateral_filter_->seqcompute(Q, n_classes_, bilateral_feats,
                                      bilateral_out);

        // Spatial message passing
        spatial_filter_->seqcompute(Q, n_classes_, spatial_feats, spatial_out);
...
void DenseCRF::mtinference(const float *unary, const float *reference,
    ...
    // == mt create feature ==
    auto tcreateft = [this, bilateral_feats, reference,
    ...
    for (int i = 0; i < n_thread_; i++) {
        ftThreads.emplace_back(tcreateft, i);
    }
    ...
    bilateral_filter_->mtcompute(allOnes, 1, bilateral_feats,
                                 bilateral_norm_vals);
    spatial_filter_->mtcompute(allOnes, 1, spatial_feats, spatial_norm_vals);
    delete[] allOnes;
    ...
    for (int i = 0; i < n_iterations_; i++) {
        ...
        // Bilateral message passing
        bilateral_filter_->mtcompute(Q, n_classes_, bilateral_feats,
                                     bilateral_out);

        // Spatial message passing
        spatial_filter_->mtcompute(Q, n_classes_, spatial_feats, spatial_out);
