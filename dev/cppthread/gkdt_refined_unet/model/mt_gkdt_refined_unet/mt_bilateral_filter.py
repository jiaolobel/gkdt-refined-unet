...
def bilateral_filter(value, image, theta_alpha, theta_beta, nthreads):
    ...
    gkdtfilter = PyGKDTFilter(n_imchannels + 2, h * w, nthreads)
    gkdtfilter.seqinit(pos)
    gkdtfilter.mtcompute(val1, n_valchannels + 1, pos, val1)
    # gkdtfilter.seqcompute(val1, n_valchannels + 1, pos, val1)
