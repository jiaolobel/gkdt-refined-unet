...
// void init(const float *pos, int pd, int n, int *splat_indices, float *splat_weights, int *splat_results, int *slice_indices, float *slice_weights, int *slice_results, int *nleaves) {
//     vector<const float *> points(n);
//     for (int i = 0; i < n; i++) {
//         points[i] = pos + i * pd;
//     }
...
//     for (int i = 0; i < n; i++) {
//         *(splat_results + i) = tree.lookup(pos + i * pd, splat_indices + i * 64, splat_weights + i * 64, 4);
...
// void filter(const float *val, int vd, int n, const int *splat_indices, const float *splat_weights, const int *splat_results, const int *slice_indices, const float *slice_weights, const int *slice_results, int nleaves, float *out) {
//     vector<float> leafValues(nleaves * vd, 0.0f);
    
//     for (int i = 0; i < n; i++) {
//         for (int j = 0; j < splat_results[i]; j++) {
//             for (int k = 0; k < vd; k++) {
//                 leafValues[splat_indices[i * 64 + j] * vd + k] += val[i * vd + k] * splat_weights[i * 64 + j];
//             }
//         }
//     }
...
class GKDTFilter {
    ...
    // }
    public:
    GKDTFilter(const float *pos, int pd, int n) {
        vector<const float *> points(n);
        for (int i = 0; i < n; i++) {
            points[i] = pos + i * pd;
        }
        ...
        this->splat_indices = new int[n * 64];
        this->splat_weights = new float[n * 64];
        this->splat_results = new int[n];

        this->slice_indices = new int[n * 64];
        this->slice_weights = new float[n * 64];
        this->slice_results = new int[n];

        for (int i = 0; i < n; i++) {
            *(this->splat_results + i) = tree.lookup(pos + i * pd, this->splat_indices + i * 64, this->splat_weights + i * 64, 4);
            *(this->slice_results + i) = tree.lookup(pos + i * pd, this->slice_indices + i * 64, this->slice_weights + i * 64, 64);
        }

        this->nleaves = tree.nLeaves();
    }
    ~GKDTFilter() {
        delete this->splat_indices, this->splat_weights, this->splat_results, this->slice_indices, this->slice_weights, this->slice_results;
        printf("Objects released.");
    }

    void filter(const float *val, int vd, int n, float *out) {
        vector<float> leafValues(this->nleaves * vd, 0.0f);
        
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < this->splat_results[i]; j++) {
                for (int k = 0; k < vd; k++) {
                    leafValues[this->splat_indices[i * 64 + j] * vd + k] += val[i * vd + k] * this->splat_weights[i * 64 + j];
                }
            }
        }
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < this->slice_results[i]; j++) {
                for (int k = 0; k < vd; k++) {
                    out[i * vd + k] += leafValues[this->slice_indices[i * 64 + j] * vd + k] * this->slice_weights[i * 64 + j];
                }
            }
        }
    }
