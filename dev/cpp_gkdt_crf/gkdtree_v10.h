...
class GKDTFilter {
    ...
    void compute(const MatrixXf &valT, MatrixXf &outT) {
        MatrixXf leafValues(vd, nleaves);
        leafValues.setZero();
        
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < splat_results[i]; j++) {
                leafValues(all, splat_indices[i * 4 + j]) += splat_weights[i * 4 + j] * valT(all, i);
            }
        }
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < slice_results[i]; j++) {
                outT(all, i) += slice_weights[i * 64 + j] * leafValues(all, slice_indices[i * 64 + j]);
            }
        }
    }
